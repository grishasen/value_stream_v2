...
bounded, normalized impression history with source chunk id and logical
customer shard, plus a transactional journal of authoritative raw fingerprints.

For each logical shard, native DuckDB SQL combines current impressions with the
rolling history and performs cross-day contact normalization, the strict
frequency window, the prior-response flag, and the scope rank. It returns only
enriched target contacts to Polars through Arrow; Polars applies count
aggregation, grouping, and provenance. Appending the
